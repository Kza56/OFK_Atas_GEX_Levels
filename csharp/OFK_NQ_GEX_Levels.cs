...
//  Lit full_levels_NQ.json et affiche tous les niveaux GEX + Options.
//
...
//
//  Panel : GEX LEVELS (run_morning_NQ.py) + Briefing (ouvre PDF)
// ============================================================================
using System;
...
namespace OFK_Suite
{
    ...
    [Category("OFK Suite")]
    [Description("Niveaux Greeks Options NQ. Lit full_levels_NQ.json.")]
    public class OFK_NQ_GEX_Levels : Indicator
    {
        ...
        public string JsonPath { get; set; } = @"C:\gex_agent\data\full_levels_NQ.json";
        ...
        public string ScriptPath { get; set; } = @"C:\gex_agent\run_morning_NQ.py";
        ...
        private void RunScript()
        {
            ...
            Application.Current?.Dispatcher?.Invoke(() =>
            {
                ...
                if (_statusText  != null) _statusText.Text = "⏳ run_morning_NQ.py en cours (CME NQ + CBOE QQQ + Claude + PDF)…";
            });
            Task.Run(() =>
            {
                try
        ...
        private void LoadLevels()
        {
            ...
            try
            {
                ...
                double spot = ParseDouble(json, "spot_nq");
                if (spot <= 0) spot = ParseDouble(json, "spot");

                double emHigh = ParseDouble(json, "range_haut_nq");
                double emLow  = ParseDouble(json, "range_bas_nq");
                double emPts  = ParseDouble(json, "expected_move_nq");
                ...
                var snap = new GexSnapshot(
                    ...
                    maxPain:     ParseDouble(json, "max_pain_nq"),
                    emHigh:      emHigh,
                    emLow:       emLow,
                    emPts:       emPts,
            ...
            out double s1, out long v1, out double s2, out long v2, out double s3, out long v3)
        {
            ...
            while (found < 3)
            {
                ...
                double sNq  = ParseDoubleInStr(obj, "strike_nq");
