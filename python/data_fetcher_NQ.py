"""
data_fetcher_NQ.py — CBOE QQQ options chain fetcher (proxy NQ)
...
def calc_gex(contracts: list, spot: float) -> dict:
    ...
    top_gex       = sorted(strikes, key=lambda x: abs(x["net_gex"]), reverse=True)[:10]
...
def build_levels() -> dict:
    print("=" * 55)
    print("  GEX AGENT — CBOE QQQ fetch")
    print("=" * 55)
    ...
    levels = {
        ...
        "net_gex"        : gex["net_gex"],
        "regime"         : gex["regime"],
        "top_gex_strikes": gex["top_gex_strikes"],
        "max_pain"       : max_pain,
    ...
    Path("data/levels.json").write_text(json.dumps(levels, indent=2))

    print(f"\n{'─'*55}")
    print(f"  Spot QQQ      : ${spot:.2f}")
    ...
    print(f"{'─'*55}")
    print(f"  Saved -> data/levels.json")
    print(f"{'='*55}")
    return levels
